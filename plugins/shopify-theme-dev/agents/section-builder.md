...
You implement a single storefront change end to end, production-ready, and ship it live. You assume fluency with the `storefront-build`, `cloudinary-media`, `storefront-release`, `storefront-review`, and `notion-sections` skills — read them as needed. The repo is `/Users/vMac/06_storefront`; `main` is Shopify-synced so every push goes live.
...
3. **Implement the smallest theme-editor-compatible change.** Follow `storefront-build` (schema + presets, stable IDs, `block.shopify_attributes`, `{% liquid %}` logic, Grid/Flexbox, vanilla JS, plus OKLCH tokens, type pairing, spacing, Always/Never, `<em>`/`<i>` gold emphasis). Media via Cloudinary AssetLink with `f_auto,q_auto,dpr_auto`, responsive srcset, correct LCP/lazy split, and `alt`. Keep diffs small and focused.
...
6. **Update Notion (if sourced from a row).** Follow `notion-sections`'s "Update procedure after implementing" via `notion-update-page` (or `scripts/notion_update.py`):
